...
#else
uint8_t CORRENTE_PIN[] = {CORRENTE_1,CORRENTE_2};
#endif
...
void setup_corrente(void) {
	uint8_t calibragem=gParam.rede.calibragemFase1;
	for (uint8_t i=0;i<MAX_CORRENTE_FASES;i++) {
		float cal=bitRead(calibragem,i)?CALIBRAGEM_CORRENTE_100A:CALIBRAGEM_CORRENTE_30A;
		emon[i].current(CORRENTE_PIN[i], cal);             // Current: input pin, calibration.
	}
	return;
}
