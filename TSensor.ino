...
,{
	...
	{0x80,1,4,5,6,A0}, // porta   0 = fim  0x80 = porta D0 (bit 7 ligado)
	#else
	#if TSENSOR_SERIE == 25
	{A1,0}, // porta   0 = fim
	#elif TSENSOR_SERIE == 46
	{A1,0}, // porta   0 = fim
	#elif TSENSOR_SERIE == 48 // RS Hospitalar
	{A0,A1}, 
	#else
	{7,8,11,12,13,0}, // porta   0 = fim
	#endif
	#endif
...
void loop(void)
{
	...
	if (bitRead(ccStatus,CC_CORRENTE)) {
		...
		#if HAS_GSM == 1
		if (!bitRead(ccStatus,CC_CWJAP) && millis()<40000) {
	...
	if (!timerWeb) {
		...
		#if HAS_GSM == 1
		if (bitRead(ccStatus,CC_CWJAP) && bitRead(ccStatus,CC_RET_HTTP)) {
			t_reboot=MAX_T_REBOOT;
	...
	//WWWW
		...
		#if HAS_CORRENTE == 1
		countCorrente=1;
		//memcpy((uint8_t*)&gDataLogger.fase1,(uint8_t*)correnteFase[0],6);
		#endif
