...
void parse_server(void)
{
 ...
 if (client = server.available())
 {
	...
	// serie
	strcat_P(data,PSTR(" s:"  xstr(TSENSOR_SERIE)  " r:"));
	//utoa(TSENSOR_SERIE,data+strlen(data),10);
	//RECURSOS
	//strcat(data,"\nr:");
	ultoa(TSENSOR_RECURSO,data+strlen(data),16);
	//sprintf_P(data+strlen(data),PSTR("\nns:%u gw:%ul"),gParam.web.eth,gParam.web.gw,gParam.web.mask);
	// ip
	//sprintf_P(data+strlen(data),PSTR("\nip:%s gw:%s mk:%s"),gParam.web.eth,gParam.web.gw,gParam.web.mask);
	#if 1
	strcat_P(data,PSTR(" ip="));
	//iptos(gParam.web.eth,data);
	strcat(data,gParam.web.ip[1]);
	//strcat(data," gw:");
	//strcat(data,gParam.web.gw);
	//strcat(data," mk:");
	//strcat(data,gParam.web.mask);
	#endif
	// server
	#if 0
	strcat(data,"\ns:");
	...
	#endif
	//sprintf_P(data+strlen(data),PSTR("\nsvr:%s:%s"),gParam.web.server_addr,gParam.web.server_port);
	// proxy
	strcat(data," y=");
